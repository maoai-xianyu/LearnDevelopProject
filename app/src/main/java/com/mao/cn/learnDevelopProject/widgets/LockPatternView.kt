...
import com.mao.cn.learnDevelopProject.utils.MathUtil
import com.mao.cn.learnDevelopProject.utils.tools.LogU

/**
 *
 * @author zhangkun
 * @time 2020-03-31 14:28
 */
...
class LockPatternView : View {
    ...
    private fun drawLine(canvas: Canvas) {
        ...
        if (mSelectPoints.size >= 1) {
            ...
            for (index in 1 until mSelectPoints.size) {

                // 两个点之间绘制一条线  sin  cos  简单讲  数学知识
    ...
    private fun initDot() {
        // 九个宫格，二维数组 [3][3]
        // 不断绘制的时候这几个点都有状态，而且后面肯定需要回调密码，点需要有下标， 点肯定是一个对象
        // 3*3 的二维数组
        LogU.d(" mPoints size " + mPoints.size)
        var width = width
        val height = height
        ...
        for (i in mPoints.indices) {
            for (j in mPoints.indices) {
                mPoints[i][j] = Point(offsetX + squareWidth * (2 * j + 1) / 2,
                        offsetY + squareWidth * (2 * i + 1) / 2,
                        i * mPoints.size + j)
            }
        }
    }
